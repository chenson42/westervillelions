...
| `members.duesCategory` | `individual \| family` (`schema.ts:44`) | **Billing-rate bucket** — which flat dues amount a member owes (individual $120 vs family $96, per `docs/features/the-ledger-accounting.md:204`). Nothing to do with LCI membership type. | `dues-queries.ts` joins `dues_settings` and picks `individualAmountCents`/`familyAmountCents` off this value | Admin, via a **separate** surface: `PATCH /api/admin/dues/[memberId]/category`, gated `FEATURES.DUES_MANAGE` — **not** the same permission that edits `membershipStatus` |
