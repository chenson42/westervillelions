...
4. **Breakdown coverage today is bounded by the still-open B-33 backlog item**, not by anything this feature controls. `countsAsGiving` is the sole eligibility switch for cause breakdown right now; B-33 (`docs/backlog.md`, unbuilt) wants to decouple "supports line-item breakdown" from "counts as philanthropic giving" so categories like Rudolph Run event costs (~$10.8k across 14 vendors) can be itemized without inflating `/members/impact`'s giving totals. Until B-33 ships, this feature will only ever show breakdown for categories already flagged `countsAsGiving` (charitable donations, grants, scholarships) — any other category the treasurer might want itemized on the report stays lump-sum regardless. Not a blocker for this feature (it correctly reflects whatever data exists), but Chris should know the ceiling is set elsewhere.
