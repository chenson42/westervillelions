...
because both real production users currently holding the `treasurer` role
also hold `board_member` or `admin`, which happens to carry `ADMIN_DASHBOARD`
