...
**Decision:** Phase 2 (Revised) architectural ruling for `docs/work-log/2026-08-21-welcome-packet-live-page.md`'s DB-backed redesign, superseding the file-based Phase 2/3 design from earlier the same day (a git-committed `docs/club-documents/*.html` read at request time), which Phase 4 discovered cannot ship: that directory is deliberately gitignored (real financial figures, `aa3b539`) and never exists in the deployed tree.

1. **Raw-HTML admin authoring is a documented, narrow exception to DECISION-076 Ruling 3** (governing documents deliberately chose markdown-only, no `rehype-raw`/raw-HTML passthrough, specifically to avoid an admin-typed-HTML-to-`dangerouslySetInnerHTML` path with no review gate). The welcome packet cannot be expressed as markdown — custom CSS variables, fixed print-pagination, embedded images, and `.box`/`.grid`/`.rule` classes are unrepresentable in the project's existing `RichMarkdownContent` pipeline — and the trust population is the smallest and most restricted this project grants for content authoring: `welcome_packet.manage`, bound to `admin` only, never `notetaker` or any broader role. This is a narrow, named exception, not a precedent for raw HTML elsewhere: **any future widening of `welcome_packet.manage` beyond `admin`, or any other feature wanting raw-HTML authoring, must revisit this decision explicitly rather than cite it as blanket cover.** No sanitization library was added — the user's explicit choice over DOMPurify — so the entire safety argument rests on the permission gate, not on content filtering.
2. **New `FEATURES` key `WELCOME_PACKET_MANAGE = "welcome_packet.manage"`**, covering create/edit/mark-current in one key (following `DOCUMENTS_MANAGE`'s precedent of author-and-publish combined, no split delete key — this feature has no delete verb at all). Bound to `admin` only in its binding migration, not `notetaker` — deliberate, since `notetaker` was purpose-built for minutes/documents-shaped secretarial work, and this project's own default rule for an unsettled new key ("bind to `admin` only until you're ready to widen it") applies as-is here, reinforced by point 1's safety argument depending on it.
3. **Two sibling tables, `welcomePackets` and `welcomePacketCurrent`** — not `documents`' parent-pointing-at-child shape, because welcome packets have no parent identity: each Lions year is a peer row, not a version of one shared document the way every `documentVersions` row belongs to one `documents` row. `welcomePackets` holds one row per Lions year (`lionsYear` label, `rawHtml` — the entire admin-authored source, unmodified, parsed at read time — `createdByUserId`/`updatedByUserId`, timestamps); no unique constraint on `lionsYear` (mirrors minutes' deliberate no-`unique(kind, meetingDate)` — a redo/correction year must stay representable); no soft-delete column (no delete verb is scoped in this feature; add one later against precedent if it's ever actually needed). `welcomePacketCurrent` is a singleton-row table (uuid PK, nullable `packetId` FK, `setByUserId`, `setAt`) following the exact convention already established by `ledgerSettings` and the acknowledgment-letter-template singleton — one row, enforced by application convention, not a fixed literal id. "Mark as current" becomes one `UPDATE welcome_packet_current SET packet_id = $id, ... WHERE id = $singletonId` — a single-row point-update that is structurally race-free (one column can only ever hold one value), unlike a boolean-per-row model, which needs "unset all others, set this one" as two logical operations even inside a single transaction. This confirms and extends the supplemental analyst's pointer-over-boolean lean, but lands on a standalone singleton-pointer table rather than a copy of `documents.currentVersionId`'s on-the-parent placement, since welcome packets have no equivalent parent row to carry it.
4. **The base64-embedded emblem image stays inline in `rawHtml`, not split out to blob storage.** Splitting it out (`@vercel/blob`, already a dependency, already used for member profile pictures) would add a new admin verb (image upload) and a new failure mode (a broken image reference if the blob moves or is deleted) for a once-a-year edit cycle whose expected authoring workflow is "edit the whole file locally, paste the whole updated file into the textarea" — the same shape the one-time migration script already uses — not incremental in-browser hand-editing of a wall of base64. Postgres has no practical row-size concern at ~375KB. If this becomes a real pain point in practice, the escalation path is `@vercel/blob` with `rawHtml` storing an `<img src>` reference instead of inline base64 — explicitly follow-up scope, not required now.
5. **Migration is a one-off `scripts/seed-welcome-packet.ts`, never a `drizzle/migrations/` entry** — same reasoning as DECISION-076 Ruling 5 (governing documents' seed) and the standing Ledger Quicken-seed memory note: a `WHERE NOT EXISTS`-style guard protects against re-insertion but not against a human re-running an edited script against a database that already has real post-seed edits on top. Follows `scripts/seed-governance-document.ts`'s exact template: dry-run default, `--apply` flag, `PROD_DATABASE_URL || DATABASE_URL || DB_URL` target resolution with a loud production banner, `SCRIPT_OPERATOR_EMAIL` attribution (never a hardcoded person, per "No Personal Data in the Repository"). Reads the still-present local (gitignored, untracked) `docs/club-documents/welcome-packet-2026-27.html` once; never re-run after `--apply` succeeds.
6. **Naming collision resolved: `src/lib/welcome-packet.ts` (singular, file-based, now superseded) is renamed to `src/lib/welcome-packets.ts` (plural, matching the new `welcomePackets` table and this project's `minutes.ts`/`documents.ts` naming-after-primary-table convention).** It keeps its still-valid pure logic (`extractPacketParts()`, `scopePacketStyles()` — confirmed input-source-agnostic by the Phase 4 ux-developer) and drops the now-dead file-resolution functions (`listWelcomePacketFiles`, `findMarkedPacketFiles`, the marker constant, the file-reading half of `resolveCurrentPacket`). A new sibling `src/lib/welcome-packets-queries.ts` holds the DB-querying replacement (`getCurrentWelcomePacket()`, `listWelcomePackets()`, `saveWelcomePacket()`, `markWelcomePacketCurrent()`), matching the `minutes.ts`/`minutes-queries.ts` and `documents.ts`/`documents-queries.ts` sibling-pair convention exactly.
7. **New admin route `/admin/welcome-packets`** (plural — overriding the supplemental analyst's tentative singular suggestion; `/admin/minutes` is the closer structural precedent, a list of many peer rows keyed by a natural period, than `/admin/documents`, which is one document's version history), added to `ADMIN_NAVIGATION`'s existing "Records" group alongside Minutes and Governing Documents, `requiredFeature: FEATURES.WELCOME_PACKET_MANAGE` — this is what makes `getAdminProtectionRules()` (DECISION-082) derive the matching proxy rule automatically. The page must still independently call `auth()` + `hasFeature()` in its own body (the proxy is a coarse outer gate, not the gate), enforced by `src/lib/admin-page-feature-gates.test.ts`. The member-facing route is unchanged: `src/app/members/records/welcome-packet/page.tsx` (singular — "the current packet," matching `/members/financial-reports`' singular framing), now fetching from `welcome-packets-queries.ts` instead of the filesystem.

**Rationale:** Every ruling extends an already-resolved pattern in this codebase (the `ledgerSettings`/acknowledgment-letter-template singleton-row convention, applied here to a genuinely parentless peer-row shape; DECISION-076 Ruling 5's one-off-script-never-a-migration reasoning; DECISION-082's nav-derived proxy protection; DECISION-041's no-DB-CHECK validated-in-app-code pattern, applied here to `lionsYear`) or makes an explicit, honestly-reasoned exception where the user made the call directly (raw-HTML authoring as a narrow, permission-gated carve-out from DECISION-076 Ruling 3, not a reversal of it).

**Impact:** New tables `welcomePackets`, `welcomePacketCurrent` in `src/lib/db/schema.ts` (placed immediately after `documentVersions`) + one idempotent migration (schema + a `welcome_packet_current` singleton-row seed). New `FEATURES` key `welcome_packet.manage`, bound to `admin` only via the `add-permission` skill. `src/lib/welcome-packet.ts` renamed to `src/lib/welcome-packets.ts` (pure logic retained, file-resolution logic removed); new `src/lib/welcome-packets-queries.ts` (DB access). New admin route `src/app/(dashboard)/admin/welcome-packets/` + an `ADMIN_NAVIGATION` entry in the existing "Records" group. Member route `src/app/members/records/welcome-packet/page.tsx` unchanged in path, changed in data source. New one-off `scripts/seed-welcome-packet.ts`. Full detail: Phase 2 (Revised) section of `docs/work-log/2026-08-21-welcome-packet-live-page.md`.
...
- Dev-DB giving total dropped substantially across 43 excluded transactions.
