...
import type { GlassesDropoffLocation } from "@/lib/db/schema";
...
export default function LocationsManager({
  ...
  initialLocations: GlassesDropoffLocation[];
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingId) {
        const res = await fetch(`/api/admin/programs/glasses-dropoff/${editingId}`, {
          method: "PATCH",
        ...
        const updated: GlassesDropoffLocation = await res.json();
        setLocations((prev) => prev.map((l) => (l.id === editingId ? updated : l)));
        toast.success("Location updated");
      } else {
        const res = await fetch("/api/admin/programs/glasses-dropoff", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
        ...
        const created: GlassesDropoffLocation = await res.json();
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/admin/programs/glasses-dropoff/${deleteId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json();
  ...
  async function toggleActive(loc: GlassesDropoffLocation) {
    setTogglingId(loc.id);
    try {
      const res = await fetch(`/api/admin/programs/glasses-dropoff/${loc.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">Manage eyeglass drop-off locations shown on the Programs page</p>
        </div>
        <button
          onClick={openAdd}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Uptown Eyecare"
